...
func (e *Endpoint) getMyIP(result *Result) {
	if myIP, exists := EndpointCache.Get("MyIP"); exists {
		result.MyIP = myIP.([]byte)
		return
	}
	...
	myIP, err := io.ReadAll(response.Body)
	if err != nil {
		result.AddError("error reading response body:" + err.Error())
	}
	EndpointCache.Set("MyIP", myIP)
	result.MyIP = myIP
}
...
// needsToRetrieveMyIP checks if there's any condition that requires MyIP lookup
func (e *Endpoint) needsToRetrieveMyIP() bool {
	for _, condition := range e.Conditions {
		if condition.hasMyIPPlaceholder() {
			return true
		}
	}
	return false
